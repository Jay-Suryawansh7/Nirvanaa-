...
const server_1 = require("../src/server"); // Will implement this next
describe('Judge Dashboard API', () => {
    test('GET /api/cases should return a list of cases', async () => {
        const response = await (0, supertest_1.default)(server_1.app).get('/api/cases');
    ...
    test('GET /api/cases should support filtering by status', async () => {
        const response = await (0, supertest_1.default)(server_1.app).get('/api/cases?status=READY');
        expect(response.status).toBe(200);
    ...
    test('GET /api/cases/metrics should return overview stats', async () => {
        const response = await (0, supertest_1.default)(server_1.app).get('/api/cases/metrics');
        expect(response.status).toBe(200);
...
//# sourceMappingURL=dashboard-api.test.js.map